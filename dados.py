...
def carregar_dados():
    ...
    df_racio = df.groupby(["Continente", "Year"])[racio].mean().reset_index().rename(columns={racio: "RacioGenero"})
    df_cresc = df.groupby(["Continente", "Year"])[crescimento].mean().reset_index().rename(columns={crescimento: "Crescimento"})
    df_idade_media = df.groupby(["Continente", "Year"])[idade_media].mean().reset_index().rename(columns={idade_media: "IdadeMedia"})
    df_taxa_alteracao_natural = df.groupby(["Continente", "Year"])[taxa_alteracao_natural].mean().reset_index().rename(columns={taxa_alteracao_natural: "TaxaAlteracaoNatural"})
    df_nascimentos = df.groupby(["Continente", "Year"])[nascimentos].sum().reset_index().rename(columns={nascimentos: "Nascimentos"})
    df_obitos = df.groupby(["Continente", "Year"])[obitos].sum().reset_index().rename(columns={obitos: "Obitos"})
    df_esperanca_vida = df.groupby(["Continente", "Year"])[esperanca_vida].mean().reset_index().rename(columns={esperanca_vida: "EsperancaVida"})
    df_esperanca_vida_homens80 = df.groupby(["Continente", "Year"])[esperanca_vida_homens80].mean().reset_index().rename(columns={esperanca_vida_homens80: "EsperancaVidaHomens80"})
    df_esperanca_vida_mulheres80 = df.groupby(["Continente", "Year"])[esperanca_vida_mulheres80].mean().reset_index().rename(columns={esperanca_vida_mulheres80: "EsperancaVidaMulheres80"})
    df_mortalidade_antes40 = df.groupby(["Continente", "Year"])[mortalidade_antes40].mean().reset_index().rename(columns={mortalidade_antes40: "MortalidadeAntes40"})
    df_mortalidade_antes60 = df.groupby(["Continente", "Year"])[mortalidade_antes60].mean().reset_index().rename(columns={mortalidade_antes60: "MortalidadeAntes60"})
    df_mortalidade_entre15e50 = df.groupby(["Continente", "Year"])[mortalidade_entre15e50].mean().reset_index().rename(columns={mortalidade_entre15e50: "MortalidadeEntre15e50"})
